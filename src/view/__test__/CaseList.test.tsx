...
describe("CaseList", () => {
  ...
  it("existing cases are cleared on filter submit", () => {
    ...
    store.dispatch(casesActionCreators.setCaseCreationStart(new Date("1/1/2018")));
    store.dispatch(casesActionCreators.setCaseCreationEnd(new Date("1/1/2019")));
    wrapper.update();

    expect(wrapper.find(CaseAgeFilter).length).toBe(1);
    wrapper.find(CaseAgeFilter).first().props().onSubmit();
